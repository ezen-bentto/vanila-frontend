export const licenseData = [
  {
    id: 1,
    licenseSrc:
      "https://www.q-net.or.kr/man001.do?id=man00103&gSite=Q&gId=&redir=rcv202.do%3Fid%3Drcv20210%26gId%3D%26gSite%3DQ",
    licenseTag: "정보통신",
    endDate: "2025.07.21",
    licenseName: "정보처리 기사 3회 필기",
    licenseSummary:
      "컴퓨터를 효과적으로 활용하기 위해서 하드웨어뿐만 아니라 정교한 소프트웨어가 필요하다. 이에 따라 우수한 프로그램을 개발하여 업무의 효율성을 높이고, 궁극적으로 국가발전에 이바지하기 위해서 컴퓨터에 관한 전문적인 지식과 기술을 갖춘 사람을 양성할 목적으로 제정됨.",
  },
  {
    id: 2,
    licenseSrc:
      "https://www.q-net.or.kr/man001.do?id=man00103&gSite=Q&gId=&redir=rcv202.do%3Fid%3Drcv20210%26gId%3D%26gSite%3DQ",
    licenseTag: "IT",
    endDate: "2025.07.19",
    licenseName: "정보처리 기사 2회 실기",
    licenseSummary:
      "컴퓨터를 효과적으로 활용하기 위해서 하드웨어뿐만 아니라 정교한 소프트웨어가 필요하다. 이에 따라 우수한 프로그램을 개발하여 업무의 효율성을 높이고, 궁극적으로 국가발전에 이바지하기 위해서 컴퓨터에 관한 전문적인 지식과 기술을 갖춘 사람을 양성할 목적으로 제정됨.",
  },
  {
    id: 3,
    licenseSrc:
      "https://www.q-net.or.kr/man001.do?id=man00103&gSite=Q&gId=&redir=rcv202.do%3Fid%3Drcv20210%26gId%3D%26gSite%3DQ",
    licenseTag: "디자인",
    endDate: "2025.06.09",
    licenseName: "웹디자인개발기능사 필기",
    licenseSummary:
      "개인 및 특정기관의 홈페이지를 제작하는 일로써, 홈페이지를 기획, 설계제작하며 이 에 따른 시스템자원 및 사용할 S/W를 활용하여 기본적인 프로그램을 수행하는 직무",
  },
  {
    id: 4,
    licenseSrc:
      "https://www.q-net.or.kr/man001.do?id=man00103&gSite=Q&gId=&redir=rcv202.do%3Fid%3Drcv20210%26gId%3D%26gSite%3DQ",
    licenseTag: "화학",
    endDate: "2025.06.09",
    licenseName: "위험물기능사 필기",
    licenseSummary:
      "위험물 취급은 위험물 안전 관리법 규정에 의거 위험물의 제조 및 저장하는 취급소에서 각 류별 위험물 규모에 따라 위험물과 시설물을 점검하고, 일반 작업자를 지시 감독하며 재해 발생 시 응급조치와 안전관리 업무를 수행하는 일",
  },
  {
    id: 5,
    licenseSrc:
      "https://www.q-net.or.kr/man001.do?id=man00103&gSite=Q&gId=&redir=rcv202.do%3Fid%3Drcv20210%26gId%3D%26gSite%3DQ",
    licenseTag: "전기",
    endDate: "2025.06.09",
    licenseName: "전기기능사 필기",
    licenseSummary:
      "전기로 인한 재해를 방지하기 위하여 일정한 자격을 갖춘 사람으로 하여금 전기기기를 제작, 제조 조작, 운전, 보수 등을 하도록 하기 위해 자격제도 제정",
  },
  {
    id: 6,
    licenseSrc:
      "https://www.q-net.or.kr/man001.do?id=man00103&gSite=Q&gId=&redir=rcv202.do%3Fid%3Drcv20210%26gId%3D%26gSite%3DQ",
    licenseTag: "정보통신",
    endDate: "2025.07.21",
    licenseName: "멀티 미디어 콘텐츠 제작전문가 필기",
    licenseSummary:
      "디지털화의 급속한 진전으로 기업, 기관 등에서 제공하는 콘텐츠들이 미디어를 통해 디지털콘텐츠로 제공됨에 따라 다양한 사용자의 요구에 맞도록 멀티미디어 콘텐츠를 제공할 수 있는 활용(기획, 디자인, 제작 등) 능력을 평가한다. ",
  },
  {
    id: 7,
    licenseSrc:
      "https://www.q-net.or.kr/man001.do?id=man00103&gSite=Q&gId=&redir=rcv202.do%3Fid%3Drcv20210%26gId%3D%26gSite%3DQ",
    licenseTag: "정보통신",
    endDate: "2025.07.21",
    licenseName: "멀티미디어콘텐츠제작전문가 필기",
    licenseSummary:
      "디지털화의 급속한 진전으로 기업, 기관 등에서 제공하는 콘텐츠들이 미디어를 통해 디지털콘텐츠로 제공됨에 따라 다양한 사용자의 요구에 맞도록 멀티미디어 콘텐츠를 제공할 수 있는 활용(기획, 디자인, 제작 등) 능력을 평가한다. ",
  },
  {
    id: 8,
    licenseSrc:
      "https://www.q-net.or.kr/man001.do?id=man00103&gSite=Q&gId=&redir=rcv202.do%3Fid%3Drcv20210%26gId%3D%26gSite%3DQ",
    licenseTag: "정보통신",
    endDate: "2025.07.21",
    licenseName: "멀티미디어콘텐츠제작전문가 필기",
    licenseSummary:
      "디지털화의 급속한 진전으로 기업, 기관 등에서 제공하는 콘텐츠들이 미디어를 통해 디지털콘텐츠로 제공됨에 따라 다양한 사용자의 요구에 맞도록 멀티미디어 콘텐츠를 제공할 수 있는 활용(기획, 디자인, 제작 등) 능력을 평가한다. ",
  },
  {
    id: 9,
    licenseSrc:
      "https://www.q-net.or.kr/man001.do?id=man00103&gSite=Q&gId=&redir=rcv202.do%3Fid%3Drcv20210%26gId%3D%26gSite%3DQ",
    licenseTag: "정보통신",
    endDate: "2025.07.21",
    licenseName: "멀티미디어콘텐츠제작전문가 필기",
    licenseSummary:
      "디지털화의 급속한 진전으로 기업, 기관 등에서 제공하는 콘텐츠들이 미디어를 통해 디지털콘텐츠로 제공됨에 따라 다양한 사용자의 요구에 맞도록 멀티미디어 콘텐츠를 제공할 수 있는 활용(기획, 디자인, 제작 등) 능력을 평가한다. ",
  },
  {
    id: 10,
    licenseSrc:
      "https://www.q-net.or.kr/man001.do?id=man00103&gSite=Q&gId=&redir=rcv202.do%3Fid%3Drcv20210%26gId%3D%26gSite%3DQ",
    licenseTag: "정보통신",
    endDate: "2025.07.21",
    licenseName: "멀티미디어콘텐츠제작전문가 필기",
    licenseSummary:
      "디지털화의 급속한 진전으로 기업, 기관 등에서 제공하는 콘텐츠들이 미디어를 통해 디지털콘텐츠로 제공됨에 따라 다양한 사용자의 요구에 맞도록 멀티미디어 콘텐츠를 제공할 수 있는 활용(기획, 디자인, 제작 등) 능력을 평가한다. ",
  },
];
